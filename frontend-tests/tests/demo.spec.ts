...
test.describe('Demo Test Suite - POM Structure', () => {
  ...
  test('Temel test - Home page title kontrolü', async () => {
    ...
    await homePage.expectTitle(/DEMOQA/);
  });
  ...
  test('Checkbox kartına tıkla ve kontrolleri yap', async () => {
    ...
    const homeCheckbox = elementsPage['page'].locator('.rct-title').filter({ hasText: 'Home' });
  ...
  test('Radio button testleri', async () => {
    ...
    await expect(elementsPage['page'].locator('h1')).toContainText('Radio Button');
    ...
    await elementsPage.expectSuccessMessageContains(testData.messages.impressive);
  ...
  test('Web tables testleri', async () => {
    ...
    await expect(elementsPage['page'].locator('h1')).toContainText('Web Tables');
    await expect(elementsPage['page'].locator('.rt-tbody')).toBeVisible();
    
    await elementsPage.clickAddButton();
    await elementsPage.expectModalHeaderContains(testData.messages.registrationForm);
    ...
    await elementsPage.expectTableBodyContains(testData.webTableForm.department);
  });
  ...
  test('Buttons testleri', async () => {
    ...
    await elementsPage.expectDoubleClickMessageContains(testData.messages.doubleClick);
    
    await elementsPage.rightClickRightClickButton();
    await elementsPage.expectRightClickMessageContains(testData.messages.rightClick);
    
    await elementsPage.clickClickMeButton();
    await elementsPage.expectDynamicClickMessageContains(testData.messages.dynamicClick);
  });
  ...
  test('Forms practice form testleri', async () => {
    ...
    await formsPage.expectElementVisible(formsPage['page'].locator('.element-list').locator('li').filter({ hasText: 'Practice Form' }));
    await formsPage.clickPracticeFormCard();
    
    await expect(formsPage['page'].locator('h1')).toContainText('Practice Form');
